...
/**
 * This decision closes a position if either the passed an absolute amount of profit was
 * reached or the account balance goes below the account close out level or if
 * the end of the trading day was reached.
 */
public class AbsoluteClose implements ClosePostionDecision {

	private final float absolute;

	public AbsoluteClose(final float absolute) {
			...
			for (int i = 0; i < ticks.size(); i++) {
				...
				if (isValid) {
					...
					float positionCosts = account.getCosts(position.price,
							position.number);
					if (profit >= (positionCosts + absolute)) {						
